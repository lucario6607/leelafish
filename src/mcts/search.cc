...
// std::queue<std::vector<Node*>> SearchWorker::PreExtendTreeAndFastTrackForNNEvaluation() {
  ...
  if(search_->search_stats_->fast_track_extend_and_evaluate_queue_.size() > 0){
    ...
    while(search_->search_stats_->fast_track_extend_and_evaluate_queue_.size() > 0 &&
	  search_->search_stats_->Number_of_nodes_added_by_AuxEngine - number_of_added_nodes_at_start < static_cast<int>(std::floor(params_.GetMiniBatchSize() * 0.3)) && 
...
bool SearchWorker::PickNodesToExtendTask(Node* node, int base_depth,
  ...
  if(override_cpuct){
	...
	if((search_->search_stats_->vector_of_moves_from_root_to_Helpers_preferred_child_node_.size() % 2) != 0){
	...
	if(act_on_first_divergence){
	  ...
	  search_->search_stats_->vector_of_moves_from_root_to_Helpers_preferred_child_node_mutex_.unlock();
	  if(params_.GetAuxEngineVerbosity() >= 2) LOGFILE << "SearchWorker::PickNodesToExtendTask() Ready with first convergence. vector_of_moves_from_root_to_Helpers_preferred_child_node_mutex_ released.";
	  return true;
	} else {
