...
bool SearchWorker::PickNodesToExtendTask(Node* node, int base_depth,
  ...
  if(override_cpuct > 0 && override_cpuct < override_cpuct_number_of_times){
	...
	if(collision_limit_one > 0){
	  ...
	  collision_limit_one = std::max(0, std::min(static_cast<uint32_t>(1024) - boosted_node->GetNInFlight(), std::min(static_cast<uint32_t>(collision_limit_one), boosted_node->GetN() - boosted_node->GetNInFlight())));
